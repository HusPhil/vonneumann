...
import { parseAssemblyCode, validateInstructions } from '../utils/assemblyParser';

interface CodeEditorProps {
  onLoadProgram: (instructions: any[], memorySetup: { address: number, value: number }[]) => void;
}
...
const CodeEditor: React.FC<CodeEditorProps> = ({ onLoadProgram }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md">
      <h3 className="text-lg font-semibold mb-2 dark:text-white">Assembly Code</h3>
      
      <div className="mb-3">
        <label htmlFor="example-select" className="block text-sm mb-1 dark:text-gray-300">
          Load Example:
        </label>
        <select
          id="example-select"
      ...
      <div className="mb-3">
        <textarea
          ...
          className="w-full h-48 p-3 border rounded font-mono text-sm bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
...
HALT      // Stop execution"
      ...
      <div className="instructions mb-3 p-3 bg-gray-50 dark:bg-gray-700 rounded text-xs">
        <h4 className="font-medium mb-1 dark:text-white">Supported Instructions:</h4>
        <ul className="grid grid-cols-2 gap-x-4 gap-y-1 text-gray-700 dark:text-gray-300">
          <li><code>LOAD addr</code> - Load from memory to ACC</li>
          <li><code>STORE addr</code> - Store ACC to memory</li>
          <li><code>ADD addr</code> - Add memory to ACC</li>
          <li><code>SUB addr</code> - Subtract memory from ACC</li>
          <li><code>JUMP addr</code> - Jump to address</li>
          <li><code>JZ addr</code> - Jump if ACC is zero</li>
          <li><code>JNZ addr</code> - Jump if ACC is not zero</li>
